...
contract GeneNFTOnChainTest is Test {
    using Strings for uint256;

    GeneNFTOnChain public geneNFT;
    ...
    function setUp() public {
        vm.startPrank(owner);
        geneNFT = new GeneNFTOnChain(owner, "Aminal Genes", "GENE");
        vm.stopPrank();
    }
    ...
    function test_MintGene() public {
        ...
        // Check gene data
        (string memory traitType, string memory traitValue, string memory svg, string memory description) = geneNFT.getGene(tokenId);
    ...
    function test_RevertWhen_MintingToZeroAddress() public {
        vm.expectRevert(GeneNFTOnChain.InvalidParameters.selector);
